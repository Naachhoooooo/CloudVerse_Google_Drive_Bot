...
import re
from .Logger import log_developer_message, log_developer_reply, log_error, get_logger
logger = get_logger()
from .database import is_admin, is_whitelisted, insert_dev_message, fetch_dev_messages, mark_dev_message_delivered
from .main import access_required
import asyncio
import sqlite3
from .config import DB_PATH

# Helper to check if user has already been notified
...
def set_user_notified(user_telegram_id):
    insert_dev_message(user_telegram_id, 'system', 'notified')
...
@access_required
async def handle_message_dev(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_command and update.message:
        await update.message.reply_text("Please enter your message for the developer.")
    elif not is_command and update.callback_query:
        await update.callback_query.edit_message_text("Please enter your message for the developer.")
    ctx.user_data["state"] = "MESSAGE_DEV"
    ...
    if ctx.user_data.get("expecting_dev_message"):
        if GROUP_CHAT_ID is None or MessageDev_TOPIC_ID is None:
            if update.message and hasattr(update.message, 'reply_text'):
                await update.message.reply_text("Developer group is not configured.")
                return
            if update.callback_query and hasattr(update.callback_query, 'edit_message_text'):
                await update.callback_query.edit_message_text("Developer group is not configured.")
                return
        try:
        ...
        if update.message and hasattr(update.message, 'from_user') and update.message.from_user:
            ...
            # Insert into dev_messages
            msg_id = insert_dev_message(user.id, 'user', text, telegram_message_id=update.message.message_id)
            ...
            log_developer_message(user.id, text)
            # Notify user only once in lifetime
            if not has_user_been_notified(user.id):
                if update.message and hasattr(update.message, 'reply_text'):
                    await update.message.reply_text("Your message has been sent to the developer.")
...
async def handle_reply_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("reply_to_user:"):
        ...
        ctx.user_data["state"] = "AWAITING_DEV_REPLY"
        await query.answer()
        await query.message.reply_text("Please type your reply to the user.")
        ...
        ctx.user_data["reply_to_msg_id"] = int(msg_id)
        ctx.user_data["state"] = "AWAITING_USER_REPLY"
        await query.answer()
        await query.message.reply_text("Please type your reply to the developer.")

# Handler for developer sending reply
async def handle_developer_reply(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if ctx.user_data is None:
        ctx.user_data = {}
    if ctx.user_data.get("state") == "AWAITING_DEV_REPLY":
        user_id = ctx.user_data.get("reply_to_user_id")
        reply_to_msg_id = ctx.user_data.get("reply_to_msg_id")
        if not user_id:
            return
        text = update.message.text or ''
        # Insert reply into dev_messages
        reply_msg_id = insert_dev_message(user_id, 'developer', text, reply_to_id=reply_to_msg_id, telegram_message_id=update.message.message_id)
        ...
        log_developer_reply(user_id, text)
        # Notify developer of delivery, then delete after 2 seconds
        delivery_msg = await update.message.reply_text("✅ Delivered to user.")
...
async def handle_user_reply(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if ctx.user_data is None:
        ctx.user_data = {}
    if ctx.user_data.get("state") == "AWAITING_USER_REPLY":
        ...
        text = update.message.text or ''
        # Insert reply into dev_messages
        reply_msg_id = insert_dev_message(user.id, 'user', text, reply_to_id=reply_to_msg_id, telegram_message_id=update.message.message_id)
...
async def handle_developer_reply_legacy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not match:
        logger.warning(f"Could not extract user ID from replied message: {replied_text}")
        log_error(f"Could not extract user ID from replied message: {replied_text}", context="handle_developer_reply")
    ...
    if update.message.text:
        ...
        log_developer_reply(user_id, update.message.text)
        username_match = re.search(r"Username: @([\w_]+)", replied_text)
        username = username_match.group(1) if username_match else 'N/A'
        logger.info(f"Forwarding developer reply to Username : @{username} ID : {user_id} message : {update.message.text}")
        ...
        logger.info(f"Forwarding developer media reply to user {user_id}")
    else:
        await ctx.bot.send_message(chat_id=user_id, text=REPLY_FROM_DEVELOPER_UNSUPPORTED)
        logger.warning(f"Developer reply to user {user_id} was unsupported type.")
        log_error(f"Developer reply to user {user_id} was unsupported type.", context="handle_developer_reply")